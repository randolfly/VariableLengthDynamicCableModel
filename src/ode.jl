...
function G(x::Vector{Float64}, dx::Vector{Float64})
    ...
    for i in 1:pulley_num
        ...
        _tmp = 0
        for j in 1:N
            _tmp += ∂ₓΦ(j, 1 - lp[i] / xe) * x[j]
        end
        _G[i+1, N+1] += -(1 - lp[i] / xe) / (rp[i] * xe) * _tmp
    end
...
function C(x::Vector{Float64}, dx::Vector{Float64})
    ...
    for i in 1:pulley_num
        ...
        _C[i+1] += 1 / rp[i] * ζ(x, dx, 1 - lp[i] / xe)
    end
